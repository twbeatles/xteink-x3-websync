...
3. 임의 웹 페이지는 타입 `css` + **선택자 도우미**(페이지 분석 → 최적 설정 적용)로 선택자를 잡을 수 있습니다.  
4. 저장 후 목록에서 활성(켜짐) 상태인지 확인합니다.
...
### 5) (선택) PC를 바꿔도 이력 유지 — 공유 데이터 폴더

OneDrive·Google Drive 등 **동기화되는 폴더**를 지정하면 사이트 목록과 전송 이력이 PC 간에 맞춰집니다.

1. 첫 실행 마법사에서 **기존 공유 폴더 연결** / **새 폴더 만들기**를 고르거나  
2. **고급 설정 → 공유 데이터 폴더**에서 폴더를 지정하고 사용을 켭니다.

exe와 같은 폴더에 `synced_posts.json`·`*설정백업*.json`이 있으면 시작 시 자동으로 이어받습니다.
...
GitHub Releases에 새 버전이 출시되면 **고급 설정 → 소프트웨어 업데이트**에서 원클릭으로 안전하게 업데이트할 수 있습니다 (Ed25519 디지털 서명 무결성 검증 및 실패 시 자동 롤백 지원).
...
| 공유 데이터 폴더 | OneDrive 등에 사이트·이력 정본 → PC를 바꿔도 새 글만 동기화 |
